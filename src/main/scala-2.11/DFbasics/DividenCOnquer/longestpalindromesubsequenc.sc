def findLPSLength(st: String) :Int = {
  return LPSAux(st, 0, st.length - 1)
  // end of method}
}


   def LPSAux(st: String, startIndex: Int, endIndex: Int): Int =
  {
    ...
    val Count2 = LPSAux(st, startIndex + 1, endIndex)
    //CASE#2 - Skip one element from beginning
    val Count3 = LPSAux(st, startIndex, endIndex - 1) //CASE#3 - Skip one element from end
