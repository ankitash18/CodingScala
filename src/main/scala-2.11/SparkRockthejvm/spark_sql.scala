...
object spark_sql extends App{
  ...
  val spark = SparkSession.builder()
    ...
    .config("spark.sql.legacy.allowCreatingManagedTableUsingNonemptyLocation", "true")//from 2.4 onwards for ovewrititng
    .getOrCreate()
...
databasesDF.show()
  ...
  stocksDF.write.mode("overwrite")
       .format("csv") //default is snappy parquet
    .saveAsTable("stocks")

  val sotcksdf2 = spark.read.table("stocks")
