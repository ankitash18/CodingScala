...
val add6and3 = add3Nums(6, _: Int, 3) //for partial functin,need to specify the type
...
def compareTriplets(xs: List[Int], compare: (Int, Int, Int) => Int)  = {
  for (triplet <- xs.sliding(3)) yield {
    compare(triplet(0), triplet(1), triplet(2))
  }
}.toArray

def compareTriplets1(xs: List[Int], compare: (Int, Int, Int) => Int) :List[Int] = {
  for (triplet <- xs.sliding(3)) yield {
    compare(triplet(0), triplet(1), triplet(2))
  }
}.toList
...
compareTriplets1(nums, add3Functionv1)
compareTriplets1(nums, add3Functionv2)
compareTriplets1(nums, add3Method)  // eta expansion
